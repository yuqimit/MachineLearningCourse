...
m = size(X, 1);  % size of the training set
n = size(X, 2);  % the size of the input
...
for i=1:num_labels
    ...
    [tempTheta(:,i)] = fmincg(@(t)(lrCostFunction(t,X,yTemp,lambda)),initialTheta,options);
    % fmincg was taken from Andrew Ng machine learning course
    all_theta(i,:) = tempTheta(:,i)';
end
